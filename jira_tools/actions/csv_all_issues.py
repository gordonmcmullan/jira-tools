...
class CsvAllIssues():
    ...
    def format_issue(issue : Issue) -> str:
        ...
        if issue.fields.resolution != None:
            formatted_issue += f",{issue.fields.resolution.name}"
        else:
            formatted_issue += ","

        formatted_issue += f",\"{iso8601.parse_date(issue.fields.created).strftime('%d/%m/%Y %H:%M')}\""
        formatted_issue += f",\"{iso8601.parse_date(issue.fields.updated).strftime('%d/%m/%Y %H:%M')}\""
        if issue.fields.resolutiondate != None: 
            formatted_issue += f",\"{iso8601.parse_date(issue.fields.resolutiondate).strftime('%d/%m/%Y %H:%M')}\""
        else:
            formatted_issue += ","
